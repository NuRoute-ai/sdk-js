import { NuRouteError } from './errors';
import type {
  AcceptInviteOptions,
  AddProviderOptions,
  ApiKey,
...
class AuthNamespace {
  ...
  async getInviteInfo(token: string): Promise<InviteInfo> {
    // Wire response is snake_case (org_name/expires_at) — this namespace already translates
    // camelCase <-> snake_case for its other methods' request bodies (signup/login above);
    // this is the same translation, just on the response side.
    const w = await this._req<Record<string, any>>('GET', '/auth/invite/info', undefined, { token });
    return { orgName: w.org_name, email: w.email, role: w.role, expiresAt: w.expires_at };
  }
...
// /v1/projects/:id/policy and /budget return the gRPC record's snake_case fields straight
// through with no transform (unlike /v1/routing/config, which the gateway does translate —
// see orgConfigToRest in routes/routing.ts). budget's PUT already sent snake_case correctly;
// policy's PUT did not convert at all, so a caller's camelCase keys were silently dropped by
// the backend. Translating both ways here, for both namespaces, makes get()/set() a
// consistent camelCase-in-camelCase-out pair, matching every other namespace in this client.

function policyFromWire(w: Record<string, any>): ProjectPolicy {
  return {
    projectId:        w.project_id,
    orgId:            w.org_id,
    allowedModels:    w.allowed_models    ?? [],
    blockedModels:    w.blocked_models    ?? [],
    allowedProviders: w.allowed_providers ?? [],
    blockedProviders: w.blocked_providers ?? [],
    allowedRegions:   w.allowed_regions   ?? [],
    maxTokens:        w.max_tokens,
  };
}

function policyToWire(p: Partial<ProjectPolicy>): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  if (p.allowedModels    !== undefined) out.allowed_models    = p.allowedModels;
  if (p.blockedModels    !== undefined) out.blocked_models    = p.blockedModels;
  if (p.allowedProviders !== undefined) out.allowed_providers = p.allowedProviders;
  if (p.blockedProviders !== undefined) out.blocked_providers = p.blockedProviders;
  if (p.allowedRegions   !== undefined) out.allowed_regions   = p.allowedRegions;
  if (p.maxTokens        !== undefined) out.max_tokens        = p.maxTokens;
  return out;
}

function budgetFromWire(w: Record<string, any>): ProjectBudget {
  return {
    projectId:    w.project_id,
    orgId:        w.org_id,
    monthlyUsd:   w.monthly_usd,
    alertPct:     w.alert_pct,
    hardLimit:    w.hard_limit,
    currentSpend: w.current_spend,
  };
}
...
class ProjectPolicyNamespace {
  ...
  async get(): Promise<ProjectPolicy> {
    return policyFromWire(await this._req('GET', `/v1/projects/${this.pid}/policy`));
  }

  set(policy: Partial<ProjectPolicy>): Promise<{ success: boolean }> {
    return this._req('PUT', `/v1/projects/${this.pid}/policy`, policyToWire(policy));
  }
}
...
class ProjectBudgetNamespace {
  ...
  async get(): Promise<ProjectBudget> {
    return budgetFromWire(await this._req('GET', `/v1/projects/${this.pid}/budget`));
  }
...
export class NuRouteClient {
  ...
  constructor(options: NuRouteClientOptions = {}) {
  ...
  private async _req<T>(
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new NuRouteError(res.status, msg, type, code);
      }

      return res.json() as Promise<T>;
    } catch (err) {
      clearTimeout(timer);
      if (err instanceof NuRouteError) throw err;
      throw new NuRouteError(0, (err as Error).message, 'network_error');
    }
  }
...
/**
 * @deprecated Use `NuRouteClient` instead. `AICPClient` will be removed in a future major
 * version. Behaves identically — this is a plain subclass with no overrides.
 */
export class AICPClient extends NuRouteClient {}
